...
if [ "$1" = "--reset" ]; then
    ...
    $JLINK $TMPSCRIPT
    rm $TMPSCRIPT
elif [ "$1" = "--prepare-NEMO" ]; then
    echo ""
    echo -e "${STATUS_COLOR}preparing for NEMO ${HEX}...${RESET}"
    echo ""
    # Write to NVMC to enable erase, do erase all, wait for completion. reset
    echo "w4 4001e504 2" > $TMPSCRIPT
    echo "w4 4001e50c 1" >> $TMPSCRIPT
    ...
    echo "r" >> $TMPSCRIPT
    # write bootloader
    echo "loadfile $BOOTLOADER" >> $TMPSCRIPT
    echo "sleep 100" >> $TMPSCRIPT
    echo "r" >> $TMPSCRIPT
    # enable write & write id
    echo "w4 4001e504 1" >> $TMPSCRIPT
    echo "w4 10001080 $ID" >> $TMPSCRIPT
    echo "sleep 100" >> $TMPSCRIPT
    echo "r" >> $TMPSCRIPT
    echo "g" >> $TMPSCRIPT
    echo "exit" >> $TMPSCRIPT
